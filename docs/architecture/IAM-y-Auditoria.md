...
Los roles y permisos antiguos (`SUPER_ADMIN` / `DATA_ADMIN` / `COMERCIAL` y `DATA_VIEW` / `DATA_MANAGE` / `OPPORTUNITY_MANAGE`) **se eliminaron** por no comprobarse en código. Usuarios reales actuales: `defru-li` y `moure-dev` (ambos ADMINISTRADOR). Los módulos de **Campañas** y **Exclusiones** siguen pospuestos (v2).
