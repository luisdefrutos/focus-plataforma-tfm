...
COLUMNAS_FINALES = [
    ...
    'CONTACT POSITION',      # <--- NUEVA COLUMNA EN LA SALIDA
    'ENTITY'
]
...
MAPEO_COMUN = {
    ...
    'ZTUEV_CONTACT_PERSON_STRUC-ABTNR_BEZ20':      'CONTACT POSITION',  # <--- NUEVO MAPEADO
...
def procesar_archivo(patron, diccionario_mapeo, codigo_entidad):
    ...
    try:
        ...
        # 4. Limpiezas puntuales
        #    - Código postal como texto (para no perder ceros)
        if 'CUSTOMER POSTAL CODE' in df.columns:
            df['CUSTOMER POSTAL CODE'] = (
                df['CUSTOMER POSTAL CODE'].astype(str)
                .str.replace(r'\.0$', '', regex=True)
                .replace('nan', '')
                .str.strip()
            )

        #    - Limpiar CONTACT POSITION (si existe)
        if 'CONTACT POSITION' in df.columns:
            df['CONTACT POSITION'] = (
                df['CONTACT POSITION']
                .astype(str)
